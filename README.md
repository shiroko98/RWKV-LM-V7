...
Long-context SFT can use `--sft_masked_ce_chunk N` to lower the head/loss memory peak. The default `0` disables this optimization and keeps the previous full-logits masked CE path. A positive value changes SFT loss so it does not materialize `[batch, ctx_len, vocab]` logits at once; it selects only target tokens where `loss_mask=1`, then computes head projection and CE in chunks of `N` trainable tokens. This matters for `ctx_len=86016`, where full logits are very large even with `micro_bsz=1`. Your 13B data has mask density around `0.821`, so this mainly lowers peak memory rather than total compute, and chunking plus checkpoint recomputation can be slower. Start with `512`, try `1024` if stable, and lower to `256` if memory is still tight.
...
8. When `--sft_masked_ce_chunk N` is positive, SFT instead uses `src/sft_loss.py::masked_head_cross_entropy`: the model body returns hidden states without first materializing full logits; the loss selects mask=1 target tokens, runs `hidden @ head.weight.T` plus CE in chunks of `N` trainable tokens, and averages the weighted loss. Each chunk is checkpointed by default, so backward recomputes chunk logits instead of storing them all. `0` disables this optimization for old-path comparison.
