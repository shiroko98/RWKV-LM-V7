...
- `SFT_TRAIN_SHUFFLE` / `SFT_TRAIN_SHUFFLE_SEED`: optional training-time SFT document shuffle. Defaults are `0` and `1234`. Set `SFT_TRAIN_SHUFFLE=1` to apply an epoch-stable deterministic train-doc permutation without changing DataLoader sharding; eval tail reading stays ordered.
