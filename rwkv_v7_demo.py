...
MODEL_PATH = "/mnt/data/Codes/RWKV/RWKV-LM-V7-12B-train/outs/12b-zero3-offload/rwkv-step-20.bf16.pth"
...
tokenizer = RWKV_TOKENIZER("/mnt/data/Codes/RWKV/RWKV-LM-V7-12B-train/data/tokenizer/rwkv_vocab_v20230424.txt")
...
with torch.no_grad():
    ...
    generated = list(input)
    first_logits = None

    for step in range(GEN_TOKENS):
        out = model.forward(torch.tensor(generated).reshape(1,-1).cuda())

        if step == 0:
            print(f'\nOutput:\n{out}')

        logits = out[0, -1].float() # logits of the last token => prediction for the next token
        probs = F.softmax(logits, dim=-1) # compute softmax in float (more accurate)
    ...
    _, indices = torch.topk(first_logits, TOP_K) # print top-k possibilities for the first generated token
    for i in range(len(indices)):
        token_id = indices[i].item()
        token = tokenizer.decode([token_id])
        token_prob = first_logits[token_id].item()
        print(token, f'[probability {token_prob:.2%}]')

    print(f'\nGenerated token ids:\n{generated[len(input):]}')
    print(f'\nGenerated text:\n{tokenizer.decode(generated[len(input):])}')
