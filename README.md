...
- `--pack-strategy`: packing strategy. Defaults to `ordered`; `best-fit-decreasing` reorders by length inside each JSONL shard, uses a best-fit approximation to reduce padding, and appends all shards into one output dataset.
