...
- `--pack-strategy`：packing 策略，默认 `ordered` 保持样本顺序；`best-fit-decreasing` 会按长度重排做近似最优打包，减少 padding。
