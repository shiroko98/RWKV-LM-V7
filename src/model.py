...
if 'x070' in os.environ["RWKV_MY_TESTING"]:
    ...
    load(name="wind_backstepping", sources=[
         f'cuda/wkv7_cuda.cu', 'cuda/wkv7_op.cpp'], is_python_module=False, verbose=True, extra_cuda_cflags=flags)
    ...
    class WindBackstepping(torch.autograd.Function):
        ...
        def forward(ctx, w, q, k, v, z, b):
            ...
            s = torch.empty(B, H, T//CHUNK_LEN, C, C,
        ...
        with torch.no_grad():
            ...
            def ortho_init(x, scale):
                with torch.no_grad():
                    ...
                    if len(shape) == 2:
                        gain = math.sqrt(
                            shape[0] / shape[1]) if shape[0] > shape[1] else 1
                        nn.init.orthogonal_(x, gain=gain * scale)
                    elif len(shape) == 3:
                        gain = math.sqrt(
                            shape[1] / shape[2]) if shape[1] > shape[2] else 1
            ...
            D_DECAY_LORA = max(
                32, int(round((1.8*(C**0.5)) / 32)*32))  # suggestion
            self.w1 = nn.Parameter(torch.zeros(C, D_DECAY_LORA))
            self.w2 = nn.Parameter(ortho_init(
                torch.zeros(D_DECAY_LORA, C), 0.1))
            ...
            self.a0 = nn.Parameter(torch.zeros(
                1, 1, C)-0.19 + zigzag*0.3 + linear*0.4)

            D_MV_LORA = max(
                32, int(round((1.3*(C**0.5)) / 32)*32))  # suggestion
            ...
            D_GATE_LORA = max(
                32, int(round((0.6*(C**0.8)) / 32)*32))  # suggestion
            self.g1 = nn.Parameter(torch.zeros(C, D_GATE_LORA))
            self.g2 = nn.Parameter(ortho_init(
                torch.zeros(D_GATE_LORA, C), 0.1))
    ...
    def forward(self, x, v_first):
        ...
        if self.layer_id == 0:
            ...
            v = torch.lerp(v, v_first, torch.sigmoid(
                self.v0 + (xv @ self.v1) @ self.v2))  # add value residual
...
class RWKV_CMix_x070(MyModule):
    def __init__(self, args, layer_id):
        ...
        self.key.weight.data.uniform_(-0.5 /
                                      (args.n_embd**0.5), 0.5/(args.n_embd**0.5))
        ...
        for n in self.state_dict():
            ...
            if "ln_" in n or ".ln" in n or "time_" in n or "_mask" in n or "pos_emb" in n or '.mask.' in n or n.endswith('_w') or n.endswith('_w1') or n.endswith('_w2') or n.endswith('_bias') or (".weight" not in n):
                ...
                zero = [".att.output.", ".ffn.value.", ".ffn.receptance.",
                        ".ffnPre.value.", ".ffnPre.receptance.", "head_q.", '.oo.', '.rr.']
